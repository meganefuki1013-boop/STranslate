...
public class Main : LlmTranslatePluginBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await foreach (var msg in Context.HttpService.StreamPostAsyncEnumerable(
            ...
            cancellationToken))
        {
            var streamEvent = GeminiProtocol.ParseStreamLine(msg);
        ...
        if (sb.Length == 0)
            throw new InvalidOperationException(Context.GetTranslation("STranslate_Plugin_Translate_Gemini_NoTextOutput"));
